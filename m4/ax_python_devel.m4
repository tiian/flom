...
variable to configure. See ``configure --help'' for reference.
	...
	if test $ax_python_devel_found = yes; then
	   ...
	   #
	   # Tiian patch, 2024-02-18, because Ubuntu 14.04 provides wrong
	   # answer to sysconfig:
	   # tiian@ubuntu1404-64:~$ python
	   ...
	   #
	   ac_sysconfig_result=`$PYTHON -c "from distutils import sysconfig" 2>&1 | grep DeprecationWarning`
	   if test $? -eq 0; then
		# import distutils generated a warning, use sysconfig if possible
		SYSCONFIG_SUFFIX=""
	   else
		# import distutils didn't generate a warning, use distutils even
		# in presence of sysconfig to bypass Ubuntu 14.04 bug
		SYSCONFIG_SUFFIX="_forced_error_suffix"
	   fi

	   ac_sysconfig_result=`$PYTHON -c "import sysconfig$SYSCONFIG_SUFFIX" 2>&1`
	   if test $? -eq 0; then
		AC_MSG_RESULT([yes])
