                              ...
                              const gchar *name)
{
    ...
    FLOM_TRACE(("flom_resource_simple_init\n"));
    TRY {
        if (NULL == (resource->name = g_strdup(name)))
            THROW(G_STRDUP_ERROR);
        FLOM_TRACE(("flom_resource_simple_init: initialized resource ('%s')\n",
                    resource->name));
    ...
    FLOM_TRACE(("flom_resource_simple_init/excp=%d/"
                "ret_cod=%d/errno=%d\n", excp, ret_cod, errno));
    return ret_cod;
}
                ...
                if (can_lock) {
                    ...
                    if (FLOM_RC_OK != (
                            ret_cod = flom_vfs_ram_tree_add_locker_conn(
                                ...
                                cl->info.lock_mode, NULL, NULL, NULL))) {
                        FLOM_TRACE(("flom_resource_simple_inmsg: unable to "
                                    "update the info in VFS for this holder "
                                    "connection\n"));
                    }                  
                    ...
                    if (can_wait) {
                        ...
                        if (FLOM_RC_OK != (
                                ret_cod = flom_vfs_ram_tree_add_locker_conn(
                                    ...
                                    peer_name == NULL ? "" : peer_name,
                                    cl->info.lock_mode, NULL, NULL, NULL))) {
                            FLOM_TRACE(("flom_resource_simple_inmsg: unable "
                                        "to "
                               ...
                               flom_conn_t *conn)
{
    ...
    TRY {
        ...
        if (FLOM_RC_OK != (
            ...
            FLOM_TRACE(("flom_resource_simple_clean: unable to "
                        "delete the info from VFS for this holder "
                        "connection\n"));
        }                  
            ...
            if (flom_resource_simple_can_lock(resource, cl->info.lock_mode)) {
                ...
                if (FLOM_RC_OK != (
                        ret_cod = flom_vfs_ram_tree_move_locker_conn(
                            cl->conn->uid))) {
                    FLOM_TRACE(("flom_resource_simple_waitings: unable to "
